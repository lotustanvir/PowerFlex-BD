  ...
  if (error && !data) {
    return (
      <section className="rounded-2xl border border-red-200 bg-white p-6 shadow-sm">
        <p className="text-sm font-medium text-red-600">Failed to load wind data: {error}</p>
        <div className="mt-3">
    ...
    <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <h2 className="text-lg font-semibold text-gray-900">
          Wind AI — Bangladesh Wind Forecast
        </h2>
        ...
        {best ? (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <p className="text-base font-semibold text-gray-900">{best.zone}</p>
            </div>
            <div>
              <p className="text-xs text-gray-400">Expected Energy (MWh / 1 MW · 24 h)</p>
              <p className="text-base font-semibold text-gray-900">
                {fmt(best.expected_energy_mwh_per_1mw_24h)}
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-400">Modeled Capacity Factor</p>
              <p className="text-base font-semibold text-gray-900">
      ...
      <div className="rounded-xl border border-gray-100 bg-gray-50 p-5 space-y-2">
        ...
        {opp ? (
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <div>
              <p className="text-xs text-gray-400">Zone</p>
              <p className="text-base font-semibold text-gray-900">{opp.zone}</p>
            </div>
            <div>
            ...
            <div>
              <p className="text-xs text-gray-400">Wind Speed @ 100 m (km/h)</p>
              <p className="text-base font-semibold text-gray-900">
                {fmt(opp.wind_speed_100m_kmh)}
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-400">Predicted Gen. (MW / 1 MW)</p>
              <p className="text-base font-semibold text-gray-900">
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-gray-100">
                {ranking
                  ...
                  .map((row) => (
                    <tr key={row.zone} className="text-gray-700">
